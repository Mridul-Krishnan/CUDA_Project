...
def create_optical_flow_gif(flow_sequence, predicted_sequence ,normal_sequence, output_file="flow_animation.gif"):
    ...
    for i, (flow, pred, rgb_image) in enumerate(zip(flow_sequence, predicted_sequence ,normal_sequence)):
        ...
        plt.savefig(flow_frame_path, bbox_inches='tight', pad_inches=0)
        ...
        plt.savefig(pred_frame_path, bbox_inches='tight', pad_inches=0)
        ...
        plt.savefig(normal_frame_path, bbox_inches='tight', pad_inches=0)
        plt.close('all')  # Close the plot
        ...
        fig, axes = plt.subplots(1, 3, figsize=(15, 10), constrained_layout =True)  # 1 row, 3 columns
        # Display each image
...
def create_depth_gif(depth_sequence, predicted_sequence ,normal_sequence, output_file="depth_animation.gif", norm_def=True):
    images = []
    ...
    for i, (depth_map, pred, rgb_image) in enumerate(zip(depth_sequence, predicted_sequence ,normal_sequence)):
        ...
        plt.savefig(depth_frame_path, bbox_inches='tight', pad_inches=0)
...
def create_gif(depth_model,flow_model,ego_model, vis_seq, norm_def=True):
    ...
    create_depth_gif(depth_sequence, warp_depth, normal_sequence,output_file="depth_animation.gif",norm_def=norm_def)
